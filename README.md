...
<head>
    ...
    <style>
        ...
        body {
            margin: 0;
            min-height: 100vh;
            background: radial-gradient(circle at top, #334e8f, #101426 55%);
            color: #f6f7fb;
        }
        main {
            max-width: 960px;
        ...
        /* 修复了原代码中 h1 同时设置了 4rem 和 1.1rem 的冲突，这里新增副标题样式 */
        .subtitle {
            font-size: 1.1rem;
            color: #cbd5ef;
            max-width: 680px;
...
<body>
    <main>
        ...
        <p class="subtitle">在线 Python 迷宫游戏平台</p>
    ...
    <script>
        ...
        levels.forEach(level => {
            ...
            // 自动计算对应的 HTML 游玩文件名
            const playHtmlFile = level.file.replace('.py', '.html'); 
            
            card.innerHTML = `
                <h2>${level.name}</h2>
                <p>${level.desc}</p>
                <div class="actions">
                    <!-- 增加了 Play 按钮，点击跳转到对应的 html 游戏页面 -->
                    <a href="${playHtmlFile}">Play</a>
